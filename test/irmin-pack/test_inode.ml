...
let test_add_inodes () =
  ...
  Alcotest.(check bool) "v1 stable" (Inode.Val.stable v1) true;
  Alcotest.(check bool) "v2 stable" (Inode.Val.stable v2) true;
  ...
  Alcotest.(check bool) "v4 not stable" (Inode.Val.stable v4) false;
  Context.close t
